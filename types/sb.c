...
int sbappend(sb *str, const char *data, size_t len) {
  ...
  if (str->size + len + 1 >= str->capacity) {
    ...
    if (!new_data) {
      return RAISE_MALLOC("could not reallocate sb data to fit new size."); // Memory allocation failed
    }
    str->data = new_data;
...
int sbinsert(sb *str, size_t pos, const char *data, size_t len) {
  if (pos > str->size) {
    return RAISE_ARGERR("cannot insert into position greater than size."); // Invalid position
  }
  if (str->size + len + 1 >= str->capacity) {
  ...
  memmove(str->data + pos + len, str->data + pos, str->size - pos + 1); // +1 to include null terminator
